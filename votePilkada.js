...
const targetDate = new Date("2024-11-27T07:00:00");
...
// Ubah nilai `requiredVotes` sesuai dengan jumlah vote yang harus masuk
...
const updateCountdown = () => {
    ...
    if (timeRemaining <= 0) {
        // Countdown finished, display voting container
...
function handleVote(event) {
    ...
    if (!selectedCards[group]) {
        ...
        totalVotes++;
        card.style.backgroundColor = '#b0e0e6'; // Highlight selected card
    } else if (selectedCards[group] !== cardId) {
        const previouslySelectedCard = document.querySelector(`[data-card="${selectedCards[group]}"]`);
        previouslySelectedCard.style.backgroundColor = ''; // Remove highlight from previous selection
        selectedCards[group] = cardId;
        card.style.backgroundColor = '#b0e0e6'; // Highlight new selected card
    }
